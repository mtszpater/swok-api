...
class UserService
{
    ...
    public function createTalk($user_login, $talk_id, $title, $start_timestamp, $room, $initial_evaluation, $event_name)
    {
        if (! $this->database->isAdmin($this->user_login, $this->user_password))
            return $this->_error("Permission denied");

        if( ! $this->database->isLoginBusy($user_login) )
            return $this->_error("User doesnt exist");

        if( ! $this->database->isEventNameBusy($event_name) )
            return $this->_error("event doesnt exist: $event_name");

        if( $this->database->existsTalk($talk_id) )
            return $this->_error("busy talk_id: $talk_id");
        /*
         * TIME RANGE
         */
        $event_start = $this->database->getStartTimeStampOfEvent($event_name);
        if( $event_start >= $start_timestamp)
            return $this->_error("bad timestamp: $start_timestamp ");
    ...
    public function rejectTalk($talk_id)
    {
        ...
        if(! $this->database->existsProposalTalk($talk_id) )
            return $this->_error("didnt found talk_id: $talk_id");
    ...
    public function registerUser($user_login, $user_password)
    {
        ...
        if( $this->database->isLoginBusy($user_login) )
            return $this->_error("busy login: $user_login");
    ...
    public function createEvent($event_name, $start, $end)
    {
        ...
        if($this->database->isEventNameBusy($event_name) )
            return $this->_error("busy eventname: $event_name");
    ...
    public function createOrganizer($user_login, $user_password, $secret)
    {
        if( ! ($secret === "d8578edf8458ce06fbc5bb76a58c5ca4") )
            return $this->_error("Wrong secret key");
    ...
    public function evaluationTalk($talk_id, $rate){
        ...
        if( ! $this->database->existsTalk($talk_id) )
            return $this->_error("talk doesnt exist: $talk_id");
    ...
    public function createProposalTalk($talk_id, $title, $start_timestamp){
        ...
        if( $this->database->existsProposalTalk($talk_id) )
            return $this->_error("busy proposal_talk_id: $talk_id");

        if( $this->database->existsTalk($talk_id) )
            return $this->_error("busy talk_id ( in column talk ): $talk_id");
    ...
    public function addFriend($user_login){
        ...
        if( $user_login === $this->user_login)
            return $this->_error("Im my friend :D");

        if( ! $this->database->isLoginBusy($user_login) )
            return $this->_error("your friend doesnt exist : $user_login");
    ...
    public function getUserPlan($limit){
        ...
        if( ! $this->database->isLoginBusy($this->user_login) )
            return $this->_error("user doesnt exist : $this->user_login");
